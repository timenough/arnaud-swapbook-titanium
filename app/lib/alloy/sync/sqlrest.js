			...
			if (this.status != 304 && this.status != 204) {
				...
				try {
					...
					/*Alloy.Globals.Logs.aalert(
						'Erreur',
						'Impossible de charger les éléments de cette page',
						'',
						'Ok',
						null,
						null,
...
Alloy.Globals.Logs.llog('-----> SQLREST URL (/lib/alloy/sync/sqlrest.js) :');
Alloy.Globals.Logs.llog(_options.url);
Alloy.Globals.Logs.llog('');
...
function Sync(method, model, opts) {
	...
	function createSQL(data, queryList) {
		...
		// Last Modified logic
		if (params.lastModifiedColumn && _.isUndefined(params.disableLastModified)) {
			values[_.indexOf(names, params.lastModifiedColumn)] = params.lastModifiedDateFormat ? moment().format(params.lastModifiedDateFormat) : moment().locale('en').utcOffset('GMT').format('ddd, D MMM YYYY HH:mm:ss ZZ');
		}
