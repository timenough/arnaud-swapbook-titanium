...
$.imgZoom = function (img, options) {
    ...
    function reset() {
        ...
        pageWidth = M.max( typeof $.browser != 'undefined' && typeof $.browser.msie != 'undefined' ?
                document.documentElement.scrollWidth :
                $(document).width(), winWidth);
        pageHeight = M.max( typeof $.browser != 'undefined' && typeof $.browser.msie != 'undefined' ?
    ...
    function zoom(duration, callback, out) {
        ...
        $(fx).animate({ pos: out ? 0 : 1 }, duration, function () {
            if (!out) {
                ...
                $wrap.filter('.' + className('width')).width(/*width*/'100%');
                $wrap.filter('.' + className('height')).height(/*height*/'100%');
                ...
                var new_height = Math.ceil($(window).width() * aspectRatio);

                $bigImg.css({ position: 'absolute', width: '100%', height: 'auto', opacity: endOpacity }).click(hideAllOverButtons);

                var $container = $wrap.find('.' + className('container'));
                $container.width(/*width*/'100%').height(new_height).prepend($bigImg);
                $wrap.css({ left: 0, top: 0 }).insertAfter(Z.svg || Z.vmlGroup)
                    .show();

                $('.' + className('next')).add($('.' + className('prev')))
                    .add($('.' + className('close'))).unbind('click');
                $('.' + className('next')).click(hideAllOverButtons);
                $('.' + className('prev')).click(hideAllOverButtons);
                $('.' + className('next') + ' span').click(nextClick);
                $('.' + className('prev') + ' span').click(prevClick);
                $('.' + className('close')).click(zoomOut);
                $('.' + className('close')).on('click',zoomOutSure);
                $('.' + className('share')).click(function(){ShareDialog($bigImg.attr('src'));});

                $wrap.css({
                    //left: endX - $container.offset().left + $wrap.offset().left,
                    top: 0/*endY - $container.offset().top + $wrap.offset().top*/,
                    zIndex: zIndex + 4
                });
    ...
    function zoomOutSure(){

        if(typeof Ti != 'undefined')Ti.App.fireEvent('app:fromWebView', {return_commande:'dezoom_on_one_image',from_id_sender:0});

        zoomOut();

    }

    function hideAllOverButtons(){

        if(hiddedAllOverButtons == false){

            $('.imgzoom-close, a.arrow, .imgzoom-share').fadeOut();
            hiddedAllOverButtons = true;

        }
        else{

            $('.imgzoom-close, a.arrow, .imgzoom-share').fadeIn();
            hiddedAllOverButtons = false;
    ...
    function ShareDialog(image){

        var img_split = image.split('/');
        var title = img_split.slice(-1).pop();

        if(typeof Ti != 'undefined')Ti.App.fireEvent('app:fromWebView', {return_commande:'share_the_image', return_file_uri:image, return_file_title:title, from_id_sender:0});
    ...
    function init() {
        ...
        setOptions(options = $.extend({
            ...
            duration: 500,
            wrapDuration: 300,
            loadingImg: "images/imgzoom-loading.gif",
            opacity: 1,
            overlayOpacity: 0.75,
            onInit: function () {}
        }, options));
